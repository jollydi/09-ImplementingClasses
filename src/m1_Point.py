...
class Point(object):
    ...
    def __repr__(self):
        x_coord = self.x
        y_coord = self.y
        string = 'Point(' + str(x_coord) + ', ' + str(y_coord) + ')'
        return string
    ...
    def move_to(self, x, y):
        new_point = Point(x, y)
        self.dist_traveled = self.dist_traveled + self.get_distance_from(new_point)
        self.x = x
        self.y = y
        self.moves = self.moves + 1

    def move_by(self, dx, dy):
        new_point = Point(self.x + dx, self.y + dy)
        self.dist_traveled = self.dist_traveled + self.get_distance_from(new_point)
        self.x = self.x + dx
        self.y = self.y + dy
        self.moves = self.moves + 1
    ...
    def get_distance_from_start(self):
        start = Point(self.init_x, self.init_y)
    ...
    def closer_to(self, p1, p2):
        dist1 = self.get_distance_from(p1)
